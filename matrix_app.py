...
def matrix_game():
    ...
    while not phone_format:
        ...
        if re.fullmatch(r'^(?:\([2-9]\d{2}\)\ ?|[2-9]\d{2}(?:\-?|\ ?))[2-9]\d{2}[- ]?\d{4}$', phone):
        #r'\d{3}-\d{3}-\d{4}'):
            phone_format = True
        else:
    ...
    matrix_1 = np.array([row_1, row_2, row_3], dtype=int)

    #print out matrix_1
    print('Your first 3 x 3 matrix is: ')
    for row in matrix_1:
        for column in row:
            print(column, end=' ')
        print()
    #matrix_1s = pd.Series([[row_1], [row_2], [row_3]], dtype=int)
    #print('Your first 3 x 3 matrix is: ' + matrix_1s)
    ...
    # create second matrix with extra precaution for array type
    matrix_2 = np.array([row_1, row_2, row_3], dtype=int)
    #print out matrix_2
    print('Your second 3 x 3 matrix is: ')
    for row in matrix_2:
        for column in row:
            print(column, end=' ')
        print()
